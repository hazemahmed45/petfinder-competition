...
from model import MobileNetV3WithMetaPawpularityClassifier
...
BACKBONE_WEIGHTS='checkpoints/pawpularity_mobilenetv3_backbone_2.pt'
...
model=MobileNetV3WithMetaPawpularityClassifier(BACKBONE_WEIGHTS)
# model=FashionMnistClassifier(train_dataset.n_classes)
model.load_state_dict(torch.load('checkpoints/pawpularity_mobilenetv3_7.pt'))
model.to(device)
...
schedular=StepLR(optimizer=optimizer,step_size=5,gamma=0.85)
